...
# Author: lododo# Robot Side Quick Configuration Script# 树莓派端快速配置脚本

# Contact: contect@lododo.org

# # For configuring ROS2 distributed environment on Raspberry Pi# 用于在树莓派上配置 ROS2 分布式环境
...
BLUE='\033[0;34m'echo "  ROS2 Distributed Setup - Robot Side"echo "  树莓派端 ROS2 分布式配置脚本"
...
read -p "Enter PC IP address: " PC_IP# Get local IP# 获取本机 IP
...
fiecho -e "${GREEN}Detected local IP: ${ROBOT_IP}${NC}"echo -e "${GREEN}检测到本机 IP: ${ROBOT_IP}${NC}"
...
# Prompt for serial port# 1. Set ROS_DOMAIN_ID# 1. 设置 ROS_DOMAIN_ID
...
# Prompt for camera serial numberecho "1. Configure ROS_DOMAIN_ID"echo "1. 设置 ROS_DOMAIN_ID"
...
echo -e "${YELLOW}Configuration Summary:${NC}"read -p "Enter ROS_DOMAIN_ID (default: 0): " DOMAIN_IDread -p "请输入 ROS_DOMAIN_ID (默认: 0): " DOMAIN_ID
...
echo "  Serial Port: $SERIAL_PORT"echo -e "${GREEN}ROS_DOMAIN_ID set to: ${DOMAIN_ID}${NC}"echo -e "${GREEN}ROS_DOMAIN_ID 设置为: ${DOMAIN_ID}${NC}"
...
if [ "$CONFIRM" != "y" ]; then
    ...
    exit 0# 2. Create DDS configuration file# 2. 创建 DDS 配置文件
...
echo ""echo "2. Create FastDDS Configuration"echo "2. 创建 FastDDS 配置文件"
...
#!/bin/bash# 3. Configure environment variables# 3. 配置环境变量

# Lododo Arm Robot Side Startup Script

# Generated by setup_robot_side.shecho "================================================"echo "================================================"



source /opt/ros/humble/setup.bashecho "3. Configure Environment Variables"echo "3. 配置环境变量"

cd ~/lododo-arm

source install/setup.bashecho "================================================"echo "================================================"
...
echo "  PC IP: $PC_IP"# Backup original .bashrc# 备份原有的 .bashrc
...
ros2 launch arm_bringup robot_side.launch.py \\
    ...
    port:=$SERIAL_PORT \\# Remove old ROS2 configuration (if exists)# 移除旧的 ROS2 配置（如果存在）
...
chmod +x ~/start_robot.sh# Add new configuration# 添加新配置
...
echo -e "${BLUE}║  Configuration Complete!                                 ║${NC}"# ROS2 Distributed Setup - Robot Side (added on $(date))# ROS2 Distributed Setup - Robot Side (添加于 $(date))
...
echo "2. Test connection to PC:"# DDS Configuration# DDS 配置
